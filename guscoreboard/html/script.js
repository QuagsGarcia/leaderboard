var admin = false
        ...
        function Addplayers(cache,identity) {
            ...
            for (const i in cache.players) {
              if (!cachefuck[cache.players[i].id]) {
                ...
                var divid = `<span class="leaderboard__name" style="float: left;position: absolute;left: -1.5vw;bottom: 0%;font-weight: 700;font-size: 14px;width: auto;min-width: 55px;background: #0808087d;border-radius: 5px;color: #daebf5;"><i class="fa-solid fa-id-badge"></i> `+cache.players[i].id+`</span>`
                if (!showid && !admin) {
                  divid = ''
                }
                var admindiv = ''
                ...
                var jobdiv = `<span class="leaderboard__name job" style="float: left;position: absolute;left: 30vw;font-weight: 700;font-size: 15px;color:`+cache.players[i].color+`;"><i class="fad fa-user-tie"></i> `+capitalizeFirstLetter(cache.players[i].job)+`</span>`
                if (!showjobs && !admin) {
                  jobdiv = ''
                }
                var discdiv = ''
                if (cache.usediscordname) {
                  discdiv = '<span class="leaderboard__name discordname" style="float: left;position: absolute;left: 3vw;font-weight: 700;font-size: 1vh;bottom: 0.5vh;"><i class="fa-brands fa-discord"></i> '+cache.players[i].discordname+'</span>'
                }
                //console.log(i,cache[i].name)
                $("#main").prepend(`<article class="leaderboard__profile">
                <img src="`+cache.players[i].avatar+`" alt="`+cache.players[i].name+`" class="leaderboard__picture">
        ...
        document.onkeyup = function (data) {
          if (data.keyCode == '27') { // Escape key 76 = L (Change the 76 to whatever keycodes you want to hide the carlock ui LINK https://css-tricks.com/snippets/javascript/javascript-keycodes/)
            $.post("https://guscoreboard/close",{},function(datab){});
            document.getElementById("scoreboard").style.display = "none";
            document.getElementById("main").innerHTML = '';
            document.getElementById("jobs").innerHTML = '';
          }
